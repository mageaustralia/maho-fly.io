...
PG_HOST="${PG_HOST:?Set PG_HOST in fly.toml env}"
PG_USER="${PG_USER:?Set PG_USER in fly.toml env}"
PG_PASS="${PG_PASS:?PG_PASS must be set as a Fly secret}"
...
if [ -z "$TABLE_COUNT" ] || [ "$TABLE_COUNT" -lt 100 ]; then
    ...
    echo "Running PHP migration (SQLite → Postgres)..."
    php /seed-postgres.php /maho-seed.db 2>&1
